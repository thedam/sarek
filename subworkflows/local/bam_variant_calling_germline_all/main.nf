...
include { BAM_JOINT_GENOTYPING_DEEPVARIANT                                             } from '../bam_joint_genotyping_deepvariant/main'
...
workflow BAM_VARIANT_CALLING_GERMLINE_ALL {
    ...
    joint_genotype                    // boolean: [mandatory] [default: false] cohort joint genotyping of DeepVariant gVCFs with GLnexus
    ...
    if (tools && tools.split(',').contains('deepvariant')) {
        ...
        // Cohort joint genotyping of the per-sample DeepVariant gVCFs with GLnexus.
        // Replaces the single-sample VCFs with one multi-sample, joint-genotyped VCF.
        if (joint_genotype) {
            BAM_JOINT_GENOTYPING_DEEPVARIANT(
                BAM_VARIANT_CALLING_DEEPVARIANT.out.gvcf,
                [ [ id:'no_region' ], [] ])

            vcf_deepvariant = BAM_JOINT_GENOTYPING_DEEPVARIANT.out.genotype_vcf
            tbi_deepvariant = BAM_JOINT_GENOTYPING_DEEPVARIANT.out.genotype_index
            versions = versions.mix(BAM_JOINT_GENOTYPING_DEEPVARIANT.out.versions)
        }
    }
